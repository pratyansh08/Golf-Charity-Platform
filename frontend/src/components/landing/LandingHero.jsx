...
function LandingHero() {
  return (
    <section className="relative h-screen min-h-[720px] overflow-hidden">
      ...
      <div className="relative mx-auto grid h-full w-full max-w-7xl items-center gap-10 px-6 pb-14 pt-24 sm:px-10 lg:grid-cols-12 lg:gap-12 lg:px-12">
        <motion.div
        ...
        >
          ...
          <h1 className="max-w-3xl text-5xl font-black leading-[0.94] tracking-tight text-white sm:text-6xl xl:text-7xl">
            One swing.
          ...
          <p className="mt-7 max-w-2xl text-base leading-relaxed text-slate-300 sm:text-xl">
          ...
          <div className="mt-10 flex flex-wrap items-center gap-4">
            <motion.div transition={{ type: "spring", stiffness: 260, damping: 20 }} whileHover={{ scale: 1.03 }}>
              <Link
                className="inline-flex items-center rounded-lg bg-red-600 px-7 py-3 text-sm font-bold text-white shadow-[0_14px_32px_rgba(220,38,38,0.45)] transition hover:bg-red-500"
                to="/signup"
              >
                Get Started
              </Link>
        ...
        <motion.div
          animate={{ y: [0, -12, 0] }}
          className="relative z-10 mx-auto hidden h-[420px] w-full max-w-[520px] lg:col-span-5 lg:block"
          transition={{ duration: 8, repeat: Infinity, repeatType: "mirror", ease: "easeInOut" }}
        >
          <div className="absolute inset-0 rounded-[2rem] bg-gradient-to-br from-red-500/30 via-fuchsia-500/20 to-violet-500/15 blur-sm" />
          <div className="absolute inset-[8%] rounded-[2rem] border border-white/10 bg-black/35 backdrop-blur-2xl" />
          <div className="absolute inset-[20%] rounded-[1.5rem] bg-gradient-to-br from-red-400/25 to-transparent" />
        </motion.div>
      </div>
    </section>
  );
